...
class GLMxxC(object):
    """
    Bluethooth Connection to laser range finder GLMxxC
    """
    device_name = ''
    socket = None
    port = 0x0001  # depends on model type
    ...
    def connect(self):
        try:
            ...
            self.connected = True
            #print('BOSCH GLMxxC connected')
        except:
            self.socket.close()
            self.conencted = False
            #print('Connecting to BOSCH GLMxxC failed')
    ...
    def find_bluetooth_services(self):
        ...
        if len(services) > 0:
            print("found %d services on %s" %
                  (len(services), self.bluetooth_address))
            print(services)
        else:
...
if __name__ == "__main__":
    ...
    # rangefinder = GLMxxC(bluetooth_address='54:6C:0E:29:92:2F')
    ...
    if rangefinder.connected:
        print('Connected ', rangefinder.device_name,
              '@', rangefinder.bluetooth_address)
        ...
        try:
            ...
            while True:
                ...
                if data == 'm':
                    ...
                    rangefinder.close()
                    print('Connection to BOSCH GLMxxC closed')
                    break
        ...
        print('Could not connect to BOSCH GLMxxC')
